...
#******************************************************************************************************
#----------------------------------------------
# PASO DE VARIABLE CUALITATIVA A CUANTITATIVA  |
#----------------------------------------------
...
#-----------------------------------
# DIAGRAMA DE CAJA MALIGNO GRAFICA |
#-----------------------------------
...
for i in range(0,nrow):
    for j in range(0,ncol):
        if (1+(i+1)*(i+j))<=len(data_pos.columns):        
            data_pos.boxplot(ax=axes21[i,j],column=data_pos.columns[1+j+i*ncol],color='red',positions=[0])
            data_neg.boxplot(ax=axes21[i,j],column=data_neg.columns[1+j+i*ncol],color='green',positions=[1])
            axes21[i,j].set_title(data_pos.columns[1+j+i*ncol])
...
for i in range(0,nrow):
    for j in range(0,ncol):
        if (1+(i+1)*(i+j))<=len(data_pos.columns):      
            data_pos.boxplot(ax=axes22[i,j],column=data_pos.columns[1+j+i*ncol],color='red',positions=[0])
            data_neg.boxplot(ax=axes22[i,j],column=data_neg.columns[1+j+i*ncol],color='green',positions=[1])
            axes22[i,j].set_title(data_pos.columns[1+j+i*ncol])
